...
```yaml post /2/users/{id}/dm/unblock
openapi: 3.0.0
info:
...
paths:
  /2/users/{id}/dm/unblock:
    post:
      ...
      summary: Unblock DMs
      description: >-
        Unblocks direct messages to or from a specific User by their ID for the
        authenticated user.
      ...
      parameters:
        - name: id
          in: path
          description: >-
            The ID of the target User that the authenticated user requesting to
            unblock dms for.
          required: true
          schema:
