...
```yaml delete /2/users/{id}/likes/{tweet_id}
openapi: 3.0.0
info:
    ...
    delete:
      ...
      parameters:
        - name: id
          in: path
          description: >-
            The ID of the authenticated source User that is requesting to unlike
            the Post.
          required: true
          schema:
...
components:
  schemas:
    UserIdMatchesAuthenticatedUser:
      type: string
      description: >-
        Unique identifier of this User. The value must be the same as the
        authenticated user.
      example: "2244994945"
    TweetId:
      type: string
