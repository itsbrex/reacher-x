...
for await (const event of result.getToolStream()) {
  switch (event.type) {
    ...
    case "preliminary_result":
      ...
      console.log(
        `Progress: ${event.result.progress}% - ${event.result.status}`
      );
      break;
  }
}
...
const tool1 = tool({
  ...
  description:
    "Search the company knowledge base for documents, FAQs, and policies. Returns relevant articles with snippets.",
  // ...
});
