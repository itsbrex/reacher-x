...
Each skill provides domain-specific instructions and capabilities.`,
  ...
  inputSchema: z.object({
    type: z
      .string()
      .describe("The skill type to load (e.g., 'pdf-processing')"),
  }),
...
${skill}`,
  ...
  execute: async (params, context) => {
    ...
    if (
      JSON.stringify(context?.turnRequest?.input || []).includes(skillMarker)
    ) {
...
const multiSkillLoader = tool({
  ...
  nextTurnParams: {
    input: (params, context) => {
      let newInput = Array.isArray(context.input)
        ? context.input
        : [context.input];
...
const skillDiscoveryTool = tool({
  ...
  execute: async ({ category }) => {
    ...
    for (const skillName of availableSkills) {
      ...
      const description =
        lines.find((l) => !l.startsWith("#")) || "No description";
