# Get User Similar Profiles - API Reference | SocialAPI

# Get User Similar Profiles - API Reference

This endpoint returns an array of user profiles that Twitter/X considers similar to a given user.

GET https://api.socialapi.me/twitter/user/{user\_id}/similar
...
```
curl "https://api.socialapi.me/twitter/user/44196397/similar" \    -H 'Authorization: Bearer API_KEY' \    -H 'Accept: application/json'
```

```
const userId = '44196397';const API_KEY = 'YOUR_API_KEY_HERE';
fetch(`https://api.socialapi.me/twitter/user/${userId}/similar`, {    method: 'GET',    headers: {        'Authorization': `Bearer ${API_KEY}`,        'Accept': 'application/json'    }}).then(response => response.json()).then(response => console.log(response)).catch(err => console.error(err));
```

```
import requests
userId = '44196397'API_KEY = 'YOUR_API_KEY_HERE'
url = f'https://api.socialapi.me/twitter/user/{userId}/similar'
...
```
$userId = '44196397';$API_KEY = 'YOUR_API_KEY_HERE';
$url = "https://api.socialapi.me/twitter/user/{$userId}/similar";
$ch = curl_init();
