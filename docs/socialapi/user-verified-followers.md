# Get User Verified Followers - API Reference | SocialAPI

# Get User Verified Followers - API Reference
...
GET https://api.socialapi.me/twitter/user/{user\_id}/verified-followers?cursor={cursor}
...
```
curl "https://api.socialapi.me/twitter/user/44196397/verified-followers" \    -H 'Authorization: Bearer API_KEY' \    -H 'Accept: application/json'
```

```
const userId = '44196397';const API_KEY = 'YOUR_API_KEY_HERE';
fetch(`https://api.socialapi.me/twitter/user/${userId}/verified-followers`, {    method: 'GET',    headers: {        'Authorization': `Bearer ${API_KEY}`,        'Accept': 'application/json'    }}).then(response => response.json()).then(response => console.log(response)).catch(err => console.error(err));
```

```
import requests
userId = '44196397'API_KEY = 'YOUR_API_KEY_HERE'
url = f'https://api.socialapi.me/twitter/user/{userId}/verified-followers'
...
```
$userId = '44196397';$API_KEY = 'YOUR_API_KEY_HERE';
$url = "https://api.socialapi.me/twitter/user/{$userId}/verified-followers";
$ch = curl_init();
