...
const tool =
  await composio.tools.getRawComposioToolBySlug("GITHUB_GET_OCTOCAT");
...
The _execute tool request_ method handles injection of the appropriate base URL and authentication credentials for the tool:
...
const createResponse = await composio.triggers.create(
  userId,
  "GITHUB_COMMIT_EVENT",
  {
    triggerConfig: {
...
function verifyWebhookSignature(req: NextApiRequest, body: string): boolean {
...
export default async function webhookHandler(
  ...
  if (payload.type === "github_star_added_event") {
    ...
    console.log(
      `Repository ${event.data.repository_name} starred by ${event.data.starred_by}`
    );
    // Add your business logic here
  }
...
Previously, the Python SDK included _[local tools](https://github.com/ComposioHQ/composio/tree/master/python/composio/tools/local)_. These were tools defined within the SDK and consisted of local shell and code-related tools such as "clipboard", "sqltool", and "shelltool".
